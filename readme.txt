...
Go to Tools > Followed Posts Tools and use the **Export tracking data** box. The detailed format gives you one row for every source post and followed post pair, which is the complete data set. The summary format gives you one row per followed post with a count, ranked most followed first. Both are CSV files that open in any spreadsheet.
