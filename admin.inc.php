...
function wherego_add_meta_box( $post_type, $post ) {
		...
		add_meta_box(
			'wherego_metabox',
			__( 'Where did they go from here', WHEREGO_LOCAL_NAME ),
...
?>
    <p>
		<label for="wherego_post_ids"><?php _e( "Followed posts' IDs:", WHEREGO_LOCAL_NAME ); ?></label>
		<input type="text" id="wherego_post_ids" name="wherego_post_ids" value="<?php echo esc_attr( $value ) ?>" size="25" />
		<em><?php _e( 'Enter a comma separated list of valid post/page IDs. Save this post to see the updated list below.', WHEREGO_LOCAL_NAME ); ?></em>
    </p>
	<?php if ( $results ) { ?>

		<h3><?php _e( 'Followed posts:', WHEREGO_LOCAL_NAME ); ?></h3>
        <ol>
		<?php
