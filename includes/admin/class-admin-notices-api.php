...
class Admin_Notices_API {
	...
	public function __construct() {
		...
		Hook_Registry::add_action( 'wp_ajax_wherego_dismiss_notice', array( $this, 'handle_notice_dismissal' ) );
	}
	...
	private function print_scripts() {
		...
		jQuery(document).ready(function($) {
			$('.notice[data-notice-id]').on('click', '.notice-dismiss', function() {
				...
				$.post(ajaxurl, {
					action: 'wherego_dismiss_notice',
					notice_id: noticeId,
					dismiss_time: dismissTime,
					nonce: '<?php echo esc_js( wp_create_nonce( 'wherego_dismiss_notice' ) ); ?>'
				});
			});
		});
		</script>
	...
	public function handle_notice_dismissal() {
		check_ajax_referer( 'wherego_dismiss_notice', 'nonce' );
